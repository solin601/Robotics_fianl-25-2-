...
from duckie_led_interfaces.srv import LedColor 
...
class LEDServiceBridge(Node):
    def __init__(self):
        ...
        self.srv = self.create_service(LedColor, '/duckie_led_control', self.led_callback)
    ...
    def led_callback(self, request, response):
        ...
        self.get_logger().info(f"LED color to: {color_cmd.upper()}")
